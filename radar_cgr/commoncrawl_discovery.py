...
def _num_pages(client: HTTPClient, api: str, prefix: str, page_size: int) -> int:
    ...
    try:
        ...
        return 1 if r.text.strip() else 0
    if isinstance(payload, dict):
        return max(0, int(payload.get("pages") or 0))
    return 0


def _page_hashes(client: HTTPClient, api: str, prefix: str, page: int, page_size: int) -> set[str]:
    r = _raw_get(client, _query_url(api, prefix, output="json", page=page, pageSize=page_size))
    if r is None:
        return set()
    hashes = set()
...
def discover(min_year: int = 2013, max_year: int = 2026, max_pages_per_query: int = 20, page_size: int = 5, max_collections: int = 0, rescan: bool = False) -> dict:
    ...
    state = _state_rows()
    indexes = _all_indexes(client, min_year, max_year)
    candidates = [x for x in indexes if rescan or state.get(x.get("id"), {}).get("status") != "SCANNED"]
    if max_collections > 0:
        candidates = candidates[:max_collections]
    ...
    for collection in candidates:
        ...
        for prefix in PREFIXES:
            ...
            try:
                ...
                collection_errors.append(f"{type(exc).__name__}: {exc}"[:350])
            time.sleep(0.45)
        if collection_errors:
            failed += 1
            errors.append({"collection": cid, "errors": collection_errors})
            state[cid] = {
                "collection_id": cid,
                "name": collection.get("name", ""),
                "from": collection.get("from", ""),
                "to": collection.get("to", ""),
                "status": "ERROR",
                "hashes_found": len(found[cid]),
                "last_scanned": now,
                "last_error": " | ".join(collection_errors)[:800],
            }
        else:
            scanned += 1
            state[cid] = {
    ...
    for ah in sorted(all_hashes):
        ...
        registry_rows.append({
            ...
            "discovery_source": prev.get("discovery_source") or ",".join(collections[:12]),
            "discovery_collections": sorted(set((prev.get("discovery_collections") or []) + collections)),
        })

    inserted, updated = upsert_jsonl("audit_registry", registry_rows, "registry_id")
    replace_jsonl("cc_collections", state.values(), "collection_id")
    remaining = sum(x.get("status") != "SCANNED" for x in state.values()) + sum(x.get("id") not in state for x in indexes)
    return {
        "run_at": now,
        "min_year": min_year,
        "max_year": max_year,
        "collections_available": len(indexes),
        "collections_attempted": len(candidates),
        "collections_scanned_ok": scanned,
        "collections_failed": failed,
        "collections_remaining": remaining,
        "queries": queries,
        "pages_read": pages_read,
        "unique_hashes_found_this_run": len(all_hashes),
        "new_registry_items": inserted,
        "updated_registry_items": updated,
        "errors": errors[:50],
        "top_collections": sorted(({"collection": cid, "unique_hashes": len(values)} for cid, values in found.items()), key=lambda x: x["unique_hashes"], reverse=True)[:30],
        "validation_rule": "Common Crawl solo descubre docIdcm; cada URL se acepta como documento únicamente si la ficha oficial CGR responde y es parseable.",
    }
...
def main() -> None:
    ...
    p.add_argument("--max-pages-per-query", type=int, default=20)
    p.add_argument("--page-size", type=int, default=5)
    p.add_argument("--max-collections", type=int, default=0, help="0 = todas las colecciones pendientes")
    p.add_argument("--rescan", action="store_true")
    args = p.parse_args()
    print(json.dumps(discover(args.min_year, args.max_year, args.max_pages_per_query, args.page_size, args.max_collections, args.rescan), ensure_ascii=False, indent=2))
