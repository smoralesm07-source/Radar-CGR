...
def run(source_filter=None,skip_network=False):
    started=iso_now();client=HTTPClient();totals={"new_documents":0,"updated_documents":0,"new_findings":0,"errors":0};discovered=set(load_seed_audits());runs=[];watch_candidates=[];tribunal_candidates=[];semaforo_result={}
    if not skip_network:
    ...
    return {"started_at":started,"totals":totals,"semaforo":semaforo_result,"temporality":temporality,"intelligence":intelligence,"resolution":resolution,"quality":quality,"watch":watch,"enforcement":enforcement,"tribunal":tribunal,"fau":fau,"parquet":parquet,"dashboard_kpis":dashboard["kpis"]}
def main():
