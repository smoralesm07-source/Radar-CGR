...
def _header_dates(table)->list[str]:
    ...
    for th in table.find_all("th"):
        text=normalize_ws(th.get_text(" ",strip=True));m=re.search(r"\((\d{2}/\d{2})\)",text)
        if m:dates.append(m.group(1))
    # El HTML CGR actual duplica cada encabezado por su versión visible/auxiliar.
    # Con 16 fechas tomamos una por pareja: 8 indicadores.
    if len(dates)>=16:return dates[::2][:8]
    return dates[:8]

def parse_semaforo(html:str,retrieved_at:str|None=None,source_url:str=SOURCE_URL)->list[dict]:
    retrieved_at=retrieved_at or datetime.now(timezone.utc).replace(microsecond=0).isoformat();snapshot_date=retrieved_at[:10];soup=BeautifulSoup(html,"lxml");best=None
    for table in soup.find_all("table"):
        rows=table.find_all("tr")
        if best is None or len(rows)>len(best.find_all("tr")):best=table
    if best is None:return []
    dates=_header_dates(best);out=[]
    for tr in best.find_all("tr"):
        cells=[normalize_ws(td.get_text(" ",strip=True)) for td in tr.find_all("td")]
        if len(cells)<10:continue
        commune,region=cells[0],cells[1];values=cells[2:10]
        if not commune or not region:continue
        for idx,((code,label),raw) in enumerate(zip(INDICATORS,values)):
            out.append({"snapshot_id":stable_id("SEM",snapshot_date,commune,code),"snapshot_date":snapshot_date,"commune":commune,"region":region,"indicator_code":code,"indicator_label":label,"indicator_as_of_ddmm":dates[idx] if idx<len(dates) else "","status":_status(code,raw),"raw_value":raw,"source_url":source_url,"retrieved_at":retrieved_at})
    return out

def persist_semaforo(rows:list[dict])->dict:
    inserted,updated=upsert_jsonl("municipal_control_snapshots",rows,"snapshot_id");municipalities=len({(x.get("commune"),x.get("region")) for x in rows});statuses={}
